  ...
  const reviews = await sql`
    ...
    return (
      <div className="max-w-xl mx-auto p-6 bg-white border border-gray-200 rounded-lg shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
        ...
        <p className="mt-2 text-lg text-gray-600">{product.product_description}</p>
        <Image className="mt-4" src={product.product_image_url} alt={product.product_name} width={500} height={100} />
        ...
        {reviews.rows.length > 0 && (
          <div className="mt-4">
            ...
            {reviews.rows.map((review) => (
              <div key={review.review_id} className="flex items-center font-style: italic">
            ...
            {product.average_rating !== "0.0" && (
              <div className="flex items-center mt-2">
                <p>Average Review:</p>
                {Array.from({ length: Math.floor(product.average_rating) }, (_, index) => (
